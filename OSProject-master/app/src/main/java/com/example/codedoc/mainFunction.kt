...
class mainFunction : AppCompatActivity() {
    ...
    private val crypto by lazy { HybridCrypto(this) }
    ...
    private companion object {
        ...
        const val CREATE_ENCRYPTED_FILE_REQUEST = 102
        const val CREATE_KEY_FILE_REQUEST = 103
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        autoCompleteTxt = findViewById(R.id.auto_complete_txt) // Выбрать алгоритм
        ...
        val algorithms = listOf("AES", "RSA/AES")
        val adapter = ArrayAdapter(this, R.layout.list_item, algorithms)
        autoCompleteTxt.setAdapter(adapter)

        autoCompleteTxt.onItemClickListener = AdapterView.OnItemClickListener { _, _, i, _ ->
            selectedAlgorithm = algorithms[i]
            Toast.makeText(this, "Выбран алгоритм: $selectedAlgorithm", Toast.LENGTH_SHORT).show()
        }
        ...
        nextButton.setOnClickListener {
            when (selectedAlgorithm) {
                "AES" -> performAESOperation()
                "RSA/AES" -> performHybridOperation()
                else -> showToast("Выберите алгоритм")
            }
        }
        /*
...
//        }
    ...
    private fun createEncryptedFileDestination() {
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            addCategory(Intent.CATEGORY_OPENABLE)
            type = "application/octet-stream"
            putExtra(Intent.EXTRA_TITLE, "encrypted_${System.currentTimeMillis()}.bin")
        }
        startActivityForResult(intent, CREATE_ENCRYPTED_FILE_REQUEST)
    }

    private fun createKeyFileDestination() {
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            addCategory(Intent.CATEGORY_OPENABLE)
            type = "application/octet-stream"
            putExtra(Intent.EXTRA_TITLE, "encryption_key_${System.currentTimeMillis()}.key")
        }
        startActivityForResult(intent, CREATE_KEY_FILE_REQUEST)
    }
            ...
            when (requestCode) {
                PICK_FILE_REQUEST -> {
                    data.data?.also { uri ->
                        ...
                        showToast("Файл выбран: ${uri.toString()}")
                    }
                }
                CREATE_ENCRYPTED_FILE_REQUEST -> {
                    val encryptedFileUri = data.data
                    createKeyFileDestination()
                    // Сохраняем Uri для последующего использования
                    encryptedFileUri?.let { pendingEncryption(it) }
                }
                CREATE_KEY_FILE_REQUEST -> {
                    val keyFileUri = data.data
                    if (isEncryptMode)
                        performEncryption(keyFileUri)
                    else
                        performDecryption(keyFileUri)
                }
            }
        }
    }

    private var pendingEncryptedFileUri: Uri? = null

    private fun pendingEncryption(encryptedFileUri: Uri) {
        pendingEncryptedFileUri = encryptedFileUri
    }

    private fun performEncryption(keyFileUri: Uri?) {
        val encryptedFileUri = pendingEncryptedFileUri ?: return
        val inputUri = fileUri ?: return

        if (keyFileUri == null) {
            showToast("Не удалось создать файл для ключа")
            return
        }

        try {
            crypto.encryptFileWithRSA(
                inputUri,
                encryptedFileUri,
                keyFileUri,
                keyPair.public
            ...
            updateFilesList(encryptedFileUri)
            updateFilesList(keyFileUri)
            showToast("Файл успешно зашифрован")
        } catch (e: Exception) {
            showToast("Ошибка шифрования: ${e.localizedMessage}")
        }
    }

    private fun showDecryptionDialog() {
        val items = filesList.map { getFileName(it) }.toTypedArray()

        AlertDialog.Builder(this)
            .setTitle("Выберите файл для дешифровки")
            .setItems(items) { _, which ->
                val selectedEncryptedUri = filesList[which]
                showKeyFileDialog(selectedEncryptedUri)
            }
            .show()
    }

    private fun showKeyFileDialog(encryptedUri: Uri) {
        val keyFiles = filesList.filter { it.toString().contains("key") }

        if (keyFiles.isEmpty()) {
            showToast("Не найден файл ключа")
            return
        }

        val items = keyFiles.map { getFileName(it) }.toTypedArray()

        AlertDialog.Builder(this)
            .setTitle("Выберите соответствующий файл ключа")
            .setItems(items) { _, which ->
                val selectedKeyUri = keyFiles[which]
                createDecryptedFileDestination(encryptedUri, selectedKeyUri)
            }
            .show()
    }

    private fun createDecryptedFileDestination(encryptedUri: Uri, keyUri: Uri) {
        val intent = Intent(Intent.ACTION_CREATE_DOCUMENT).apply {
            addCategory(Intent.CATEGORY_OPENABLE)
            type = "*/*"
            putExtra(Intent.EXTRA_TITLE, "decrypted_${System.currentTimeMillis()}.txt")
        }
        startActivityForResult(intent, CREATE_ENCRYPTED_FILE_REQUEST)
        // Сохраняем Uri для последующего использования
        pendingDecryption = Pair(encryptedUri, keyUri)
    }

    private var pendingDecryption: Pair<Uri, Uri>? = null

    private fun performDecryption(decryptedFileUri: Uri?) {
        val (encryptedUri, keyUri) = pendingDecryption ?: return

        if (decryptedFileUri == null) {
            showToast("Не удалось создать файл для расшифровки")
            return
        }

        try {
            crypto.decryptFileWithRSA(
                encryptedUri,
                decryptedFileUri,
                keyUri,
                keyPair.private
            ...
            updateFilesList(decryptedFileUri)
            showToast("Файл успешно расшифрован")
        } catch (e: Exception) {
            showToast("Ошибка дешифровки: ${e.localizedMessage}")
        }
    }

    private fun updateFilesList(newUri: Uri) {
        if (!filesList.contains(newUri)) {
            filesList.add(newUri)
            adapter.clear()
            adapter.addAll(filesList.map { getFileName(it) })
    ...
    private fun performAESOperation() {
        currentStage = 2
        uploadButton.visibility = View.GONE
        nextButton.visibility = View.GONE
        keyInputSection.visibility = View.VISIBLE
...
//                Toast.makeText(this, "Введите ключ", Toast.LENGTH_SHORT).show()
    ...
    private fun performHybridOperation() {
        fileUri?.let { uri ->
            showToast("Гибридное шифрование")
            if (isEncryptMode) {
                uploadButton.visibility = View.GONE
                nextButton.visibility = View.GONE
                downloadSection.visibility = View.VISIBLE
                createEncryptedFileDestination()
            } else
                showDecryptionDialog()
            //createDecryptedFileDestination()
